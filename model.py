    ...
    def loginInsert1(self, usu1, pswd1, usu1Input, pswd1Input):
        try:
            ...
            for (cod, usu1, pswd1) in self.con:
                if usu1 == usu1Input and pswd1 == pswd1Input:
                    return True
            else:
    ...
    def loginInsert(self, usu, pswd, usuInput, pswdInput):
        try:
            ...
            for (cod, usu, pswd) in self.con:
                if usu == usuInput and pswd == pswdInput:
                    return True
            else:
                print('\nUsuário ou senha incorretos!')
            return False
...
#Aba Agenda
    ...
    def cadastroAgenda(self, dataExame,lugarExame,tipoExame ):
        try:
            sql = "Insert into abaAgenda(cod, dataExame, lugarExame, tipoExame) values('','{}','{}','{}')".format(dataExame, lugarExame, tipoExame)
            self.con.execute(sql)
            self.db_connection.commit()
            return "{} Inserido!".format(self.con.rowcount)
        except Exception as erro:
    ...
    def consultarAgenda(self, cod):
        try:
            sql = "select * from abaAgenda where cod ='{}'".format(cod)
            self.con.execute(sql)
            msg = ""         
            for(dataExame,tipoExame, lugarExame) in self.con:
                msg = msg + "\nCódigo: {}, Data do Exame: {}, Local do Exame: {}, Tipo do Exame: {}".format(dataExame, lugarExame, tipoExame)
            return msg
        except Exception as erro:
            return erro
